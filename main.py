...
if __name__ == "__main__":
	...
	try:
		...
		if asr_req.response.was_successful():
			 ...
			 i = 1
			 #print(results["search"]["ayas"][i])
			 while i < len(results["search"]["ayas"]):
			 	print(results["search"]["ayas"][i]["sura"]["name"])
			 	print(results["search"]["ayas"][i]["sura"]["id"])
			 	print("verse" , results["search"]["ayas"][i]["aya"]["id"])
			 	i = i + 1
			 	pass
