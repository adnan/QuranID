...
import os, sys, wave, requests, time, csv,codecs,cStringIO
...
class UnicodeWriter:
    ...
    def writerow(self, row):
    	...
    	for i,c in enumerate(row):
    		ret += c
        self.writer.writerow(ret.encode("utf-8"))
...
class ASRResponse(NDEVResponse):
	...
	def get_recognition_result(self):
		...
		ret = u""
		for i,c in enumerate(self.results[0]):
			#i = i.join(c)
			ret += c
			#print(i,c)
		print ret
		...
		"""
		b = open('test.csv', 'w')
		w = csv.writer(b, dialect=csv.excel, **kwds)
		w.writerow(ret.encode("utf-8"))
		"""
...
class ChunkedASRRequest(NDEVRequest):
	...
	def __init__(self, language, credentials=None):
		...
		#"https://dictation.nuancemobility.net/NMDPAsrCmdServlet/dictation?"
	...
	def analyze(self, readstream):
		...
		#print(hdrs)
		#print(url)
		#print(readstream(self.filename))
